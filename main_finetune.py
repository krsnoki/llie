...
from utils import parse_args, RainDataset, rgb_to_y, psnr, ssim, VGGPerceptualLoss, Blur,LossNetwork

blur = Blur(3).cuda()
cl = torch.nn.L1Loss(reduction='mean')
vgg_model = vgg16(pretrained=True).features[:16]
vgg_model = vgg_model.cuda()
# vgg_model = nn.DataParallel(vgg_model, device_ids=device_ids)
for param in vgg_model.parameters():
    param.requires_grad = False
perceptual_loss = LossNetwork(vgg_model)#VGGPerceptualLoss().cuda()
...
if __name__ == '__main__':
    ...
    model = Restormer(args.num_blocks, args.num_heads, args.channels, args.expansion_factor).cuda()
    if args.model_file:
        if args.finetune is True:
            ...
            for n_iter in train_bar:
                ...
                rain, norain = rain.cuda(), norain.cuda()
                out = model(rain)
                loss = 10*F.l1_loss(out, norain) + 30*perceptual_loss(out, norain) + 10*cl(blur(out), blur(norain))
